...
class BO:
	...
	EI = np.zeros(X.shape[0])
        mu, variance = self.predict(X,predict_variance=True)  
        sigma = np.sqrt(variance)
	
	for i in xrange(0,len(X)):
            #print 'mu='+str(mu[i])+';  sigma='+str(sigma[i])
	    xi = 0.0
	    Z=1.*(mu[i]-self.best_value-xi) / (sigma[i])
	    EI[i]=(mu[i]-self.best_value-xi)*stats.norm.cdf(Z)+(sigma[i])*stats.norm.pdf(Z)
